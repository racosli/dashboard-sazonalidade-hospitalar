...
df_g  = df[df["grupo_cid"]==grupo_sel].copy()
serie = df_g.groupby(["ano","mes"])["aih"].sum().reset_index()
...
fig_dec.add_trace(go.Scatter(
    x=serie["periodo"], y=serie["aih"], name="AIH observada",
    mode="lines", line=dict(color="#90CAF9", width=1),
    fill="tozeroy", fillcolor="rgba(144,202,249,0.1)"
), row=1, col=1)
fig_dec.add_trace(go.Scatter(
    x=serie["periodo"], y=serie["tendencia"], name="Tendência MM12",
    mode="lines", line=dict(color="#E53935", width=2.5)
), row=1, col=1)

cores_f = ["#E53935" if v>1.05 else "#4CAF50" if v<0.95 else "#FB8C00"
           for v in serie["fator_saz"].fillna(1)]
fig_dec.add_trace(go.Bar(
    x=serie["periodo"], y=serie["fator_saz"],
    name="Fator sazonal", marker_color=cores_f, opacity=0.7
), row=2, col=1)
fig_dec.add_hline(y=1.0, line_dash="dash", line_color="gray", row=2, col=1)

cores_i = ["#E53935" if v>105 else "#4CAF50" if v<95 else "#FB8C00"
           for v in idx_pct.values]
fig_dec.add_trace(go.Bar(
    x=[MESES_N[m] for m in idx_pct.index], y=idx_pct.values,
    name="Índice sazonal", marker_color=cores_i,
    text=[f"{v:.1f}" for v in idx_pct.values], textposition="outside"
), row=3, col=1)
fig_dec.add_hline(y=100, line_dash="dash", line_color="gray", row=3, col=1)

fig_dec.update_layout(height=750, hovermode="x unified")
...
st.info(f"📌 **{grupo_sel}** · Pico: **{MESES_N[pico]}** ({idx_pct[pico]:.1f}%) · "
...
with col_r:
    ...
    for i, g in enumerate(grupos_analisar):
        ...
        mss  = [MESES_N[m] for m in sub["mes"]] + [MESES_N[sub["mes"].iloc[0]]]
        fig_radar.add_trace(go.Scatterpolar(
            r=vals, theta=mss, fill="toself", name=g,
            line=dict(color=cores_r[i%len(cores_r)], width=2),
            fillcolor=cores_r[i%len(cores_r)], opacity=0.15
        ))
    fig_radar.update_layout(
        polar=dict(radialaxis=dict(visible=True, range=[60,145])),
        title="Radar de sazonalidade", height=420
    )
...
for g in GRUPOS:
    ...
    if a16 > 0 and a24 > 0:
        ...
        cagr_data.append({
            "grupo_cid":g,"aih_2016":a16,"aih_2024":a24,
            "cagr":round(cagr,1),"var_total":round((a24/a16-1)*100,1)
        })
...
with col_c1:
    ...
    fig_cagr = go.Figure(go.Bar(
        x=df_cagr["cagr"], y=df_cagr["grupo_cid"], orientation="h",
    ...
    fig_cagr.update_layout(
        ...
        xaxis_title="Taxa crescimento anual (%)", height=500, showlegend=False
    )
...
with col_c2:
    ...
    df_rank = df_cagr[["grupo_cid","cagr","var_total"]].sort_values(
        "cagr", ascending=False).reset_index(drop=True)
    df_rank.index += 1
    df_rank.columns = ["Grupo CID-10","CAGR (% aa)","Var. Total (%)"]
    st.dataframe(df_rank, height=450)

# Regressão
st.markdown(f"**Regressão linear — {grupo_sel}**")
...
fig_reg.add_trace(go.Bar(
    x=df_reg["ano"], y=df_reg["aih"], name="AIH observada",
    marker_color=["#F44336" if a in anos_excluir else "#2196F3" for a in df_reg["ano"]],
    opacity=0.7
))
...
fig_anom.add_trace(go.Scatter(
    x=df_anom["periodo"], y=df_anom["media"]+2*df_anom["std"],
    mode="lines", name="+2σ", line=dict(color="rgba(244,67,54,0.3)",width=1,dash="dot")
))
fig_anom.add_trace(go.Scatter(
    x=df_anom["periodo"], y=df_anom["media"]-2*df_anom["std"],
    mode="lines", name="-2σ", fill="tonexty",
...
if not anom.empty:
    tab_a = anom[["ano","mes","aih","media","z"]].copy()
    tab_a["mes"]   = tab_a["mes"].map(MESES_N)
    tab_a.columns  = ["Ano","Mês","AIH observada","Média histórica","Z-score"]
    tab_a["AIH observada"]   = tab_a["AIH observada"].apply(lambda x: f"{int(x):,}")
    tab_a["Média histórica"] = tab_a["Média histórica"].apply(lambda x: f"{int(x):,}")
    tab_a["Z-score"]         = tab_a["Z-score"].apply(lambda x: f"{x:+.2f}σ")
    st.dataframe(tab_a.sort_values("Z-score",ascending=False))

st.markdown("---")

# ════════════════════════════════════════════════════════════════
# SEÇÃO 5 — PERFIL SAZONAL
# ════════════════════════════════════════════════════════════════
...
fig_perf.add_trace(go.Scatter(
    x=[MESES_N[m] for m in sorted(med_hist.index)], y=med_hist.values,
...
pivot_corr = df_nc.groupby(["mes","grupo_cid"])["aih"].mean().unstack("grupo_cid")
...
fig_corr.update_layout(
    title="Correlação sazonal entre grupos CID-10",
...
c1, c2 = st.columns(2)
with c1:
    st.success(f"**{grupo_sel}** tem sazonalidade similar a:\n\n" +
               "\n".join([f"- **{g}**: {v:.2f}" for g,v in corr_g.head(3).items()]))
with c2:
    st.warning(f"**{grupo_sel}** tem sazonalidade oposta a:\n\n" +
